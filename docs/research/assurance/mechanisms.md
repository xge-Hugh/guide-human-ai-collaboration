...
> 实验设计与试点结论：[human-ai-collaboration-v1-implementation.md](studies/v1/assurance-v1.md)  
...
5. 具体实验栈与结果读 [v1 实施笔记](studies/v1/assurance-v1.md)，不要把本文当成安装说明
...
- **实验用途**：`SKILL.md` 只保存路由和最低充分规则；完整规范作为按需参考（现为 [docs/norm/](../../spec/README.md)）。
...
这些内容构成保障形式实验区别于现有「提高 Agent 交付率」框架的核心假设；规范正文中的原则与机制见 [docs/norm/](../../spec/README.md)。
...
规范对应：[五阶段](../../spec/workflows.md) 与原则中的先例条款；Skill：`references` 中的阶段检查。
...
规范对应：[复盘阶段](../../spec/workflows.md)；Skill：`review-and-retrospective.md`。
...
规范对应：[风险、验证与回退](../../spec/norms.md)；Skill：`review-and-retrospective.md`。
...
规范对应：[自然对话、共同理解与表示路由](../../spec/norms.md)；候选动态：[共同推进](../../../insights/2026-08-12-shared-conversational-initiative.md)；Skill：`interaction-and-learning.md`。
